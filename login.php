...
require CONNECT_PATH; // call_func/connect.php
...
$token_1 = $csrf->validate('token_login_admin',$_POST['token_login_admin_forms']);
if($token_1) {
	// continue...
}else{
	// if token not found
...
if(isset($_POST['username']) AND isset($_POST['password']) AND isset($_POST['submit']) AND $_POST['submit'] =="login" ){
	...
	if(!empty($username) AND !empty($password)){
		...
		// if username and password are not exist in database
		...
		// if there's an error - neru
		if($num_row == 0 ){
			$msg_response['status']="error";
			$msg_response['msg']="Incorrect username";
			// send error msg to index(login page)
			$session_class->setValue('error',$msg_response['msg']);
			header('Location: '.BASE_URL.'index.php');
			exit();
		}else if( $num_row > 0 ) { 
			$name =$row['firstname']." ".$row['lastname']; // user full name
			$fname = $row['firstname']; // user first 
			$mname = $row['middlename']; // user middle name
			$lname = $row['lastname']; // user last name
			...
			if($row['status']==1){
				...
				if($row['password'] != $password){
					...
					$error_login =true;
					$msg_response['status']="error";
					$msg_response['msg']="Incorrect Password";
				...
				// locked account from many login attemp
				if($row['locked'] == 1){
					$msg_response['status']="error";
					...
					$session_class->setValue('user_id',$row['user_id']); // user id
					$session_class->setValue('role_id',array('ADMIN')); // if role is equal to array('ADMIN')
					$session_class->setValue('photo',$row['location']); // Photo
					$session_class->setValue('name',$name); // user fullname
					$session_class->setValue('fname',$fname); // user first name
					$session_class->setValue('mname',$mname); // user middle name
					$session_class->setValue('lname',$lname); // user last name
